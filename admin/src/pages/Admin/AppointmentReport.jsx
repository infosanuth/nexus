...
const AppointmentReport = () => {
  ...
  const { aToken, appointmentReport, getAppointmentReport } = useContext(AdminContext)
  ...
  const doctorNames = [...new Set(appointmentReport.map((item) => item.doctorName?.trim()).filter(Boolean))].sort()
  const doctorSearchResults = doctorNames.filter((name) => name.toLowerCase().includes(search.trim().toLowerCase()))

  const filteredReport = useMemo(() => {
    const term = search.trim().toLowerCase()
    if (!term) return appointmentReport
    return appointmentReport.filter((item) => item.doctorName.toLowerCase().includes(term))
  }, [appointmentReport, search])
  ...
  const handleExport = () => {
    const header = ['Doctor', 'Total Appointment', 'Upcoming Appointment', 'Complete Appointment', 'Cancel Appointment', 'Earnings', 'Profit']
    const rows = filteredReport.map((item) => [
      item.doctorName,
      item.totalAppointments,
      item.upcomingAppointments,
      item.completeAppointments,
      item.cancelAppointments,
      item.earnings,
      item.profit
    ])
    ...
    ws['!cols'] = [
      ...
      { wch: 18 }, // Upcoming Appointment
      { wch: 18 }, // Complete Appointment
      { wch: 17 }, // Cancel Appointment
      { wch: 14 }, // Earnings
      { wch: 14 }, // Profit
    ]
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Appointment Report')
    XLSX.writeFile(wb, `appointment-report-${new Date().toISOString().slice(0, 10)}.xlsx`)
  }

  return (
    <div className='w-full max-w-6xl m-5'>

      <p className='mb-3 text-lg font-medium'>Appointment Report</p>
      ...
      <div className='overflow-hidden bg-white border rounded-xl text-sm max-h-[80vh] overflow-y-auto'>
        <div className='max-sm:hidden grid grid-cols-[1.8fr_1.1fr_1.4fr_1.4fr_1.3fr_1fr_1fr] gap-1 py-3 px-6 border-b bg-gray-50 text-[11px] font-semibold text-gray-400 uppercase tracking-wider'>
          <p>Doctor</p>
          <p className='text-center'>Total Appointment</p>
          <p className='text-center'>Upcoming Appointment</p>
        ...
        {filteredReport.length === 0
          ...
          : paginatedReport.map((item) => (
            <div
              className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[1.8fr_1.1fr_1.4fr_1.4fr_1.3fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b last:border-0'
              key={item.doctorId}
            >
              <p className='font-medium text-gray-800'>{item.doctorName}</p>
              <p className='text-center'>{item.totalAppointments}</p>
              <p className='text-center'>{item.upcomingAppointments}</p>
              <p className='text-center'>{item.completeAppointments}</p>
              <p className='text-center'>{item.cancelAppointments}</p>
