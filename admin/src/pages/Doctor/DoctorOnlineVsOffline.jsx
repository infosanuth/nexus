...
const DoctorOnlineVsOffline = () => {
  ...
  const data = useMemo(() => {
    ...
    const online = paid.filter((item) => !item.isWalkIn).length
    const walkIn = paid.length - online
    return [{ name: 'Online', value: online }, { name: 'Walk-in', value: walkIn }]
  }, [appointments])

  const [online, walkIn] = data.map((item) => item.value)
  const total = online + walkIn

  return (
    <div className='w-full max-w-6xl m-5'>
      <p className='mb-4 text-lg font-medium'>Online vs Offline</p>
      ...
      <div className='grid grid-cols-1 gap-4 mb-6 sm:grid-cols-3'>
        ...
        <div className='p-4 bg-white border border-gray-200 rounded-xl'>
          <p className='text-xl font-semibold text-gray-800'>{walkIn}</p>
          <p className='text-xs text-gray-500'>Walk-In Appointment</p>
        </div>
      </div>
...
export default DoctorOnlineVsOffline
