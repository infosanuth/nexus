...
// A past session is always ended, so any appointment that's neither
// completed nor cancelled means the patient never showed up
const getAppointmentStatus = (item) => {
  if (item.cancelled) return 'Cancelled'
  if (item.isCompleted) return 'Completed'
  return "Patient Didn't Come"
}

const ReceptionSessionAppointmentsHistory = () => {

  const { sessionId } = useParams()
  const navigate = useNavigate()
  const { rToken, sessionDetails, sessionAppointments, getSessionAppointments } = useContext(ReceptionContext)
  const { calculateAge } = useContext(AppContext)
  const [search, setSearch] = useState('')

  useEffect(() => {
    if (rToken && sessionId) {
      getSessionAppointments(sessionId, true)
    }
  }, [rToken, sessionId])
  ...
  return (
    <div className='w-full max-w-6xl m-5'>
      ...
      <div className='flex flex-wrap items-center gap-3 mb-4'>
        <button
          onClick={() => navigate('/reception-session-history')}
        ...
        <div>
          <p className='text-lg font-semibold text-gray-800'>Session Appointments History</p>
          {sessionDetails && (
...
export default ReceptionSessionAppointmentsHistory
